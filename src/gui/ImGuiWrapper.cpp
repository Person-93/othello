...
ImGuiWrapper::ImGuiWrapper(std::string_view title) {
  ...
  if (glfwWindow) {
    ...
    THROW_EXCEPTION(Exception{} << Because(
                        "There can only be one instance of ImGuiWrapper"));
  }
  if (!clear_color)
  ...
  if (gl3wInit()) {
    ...
    std::ostringstream ss;
    ss << "Failed to initialize OpenGL loader. Version is: " << version.major
       << '.' << version.minor;
...
ImGuiWrapper::Frame::Frame(int fps) : fps{fps} {
  ...
  if (inFrame) {
    ...
    THROW_EXCEPTION(
        Exception{} << Because(
            "Cannot start new frame until previous frame is completed"));
  }
  inFrame = true;
