...
public:
  ...
  StringBuilderBuffer &
  operator=(StringBuilderBuffer &&) noexcept(false) = default;

protected:
  std::streamsize xsputn(const Char *s, std::streamsize count) override {
    if (const auto spaceRemaining = this->epptr() - this->pptr();
        count > spaceRemaining)
      count = spaceRemaining;
    const StringView stringView(
        s, static_cast<typename StringView::size_type>(count));
    string += stringView;
...
/**
 * A utility for building strings on the fly that implicitly converts to a
 * string when it's finished.
 *
 * Designed to allow minimal dynamic memory allocation if needed. If it's not,
 * just use the default constructor.
 *
 * THere are two constructors, one takes a size_t and constructs a string with
 * that amount of reserved memory, the other takes a reference to a string and
 * uses that string as its buffer. The second constructor is intended to be used
 * when a StringBuilder will be created repeatedly in a loop, by making a
 * std::string outside the loop, there will be less dynamic memory allocation.
 *
 * @tparam Char The character type for the string, defaults to char
 * @tparam Traits The character traits of the string, defaults to
...
public:
  /**
   * Makes a StringBuilder object with the underlying string starting with space
   * for <code>buffSize</code> characters.
   *
   * @param buffSize
   */
  explicit StringBuilder(typename String::size_type buffSize = 16)
      : string(detail::string<Char, Traits, Allocator>(buffSize)),
        buffer(std::get<String>(string)), ostream(&buffer) {}
   ...
   * @param string
   */
  explicit StringBuilder(String &string)
      : string(&string), buffer(*(std::get<String *>(this->string))),
        ostream(&buffer) {
    string.clear();
  }
  ...
  operator<<(std::basic_ostream<Char_, Traits_> &out,
